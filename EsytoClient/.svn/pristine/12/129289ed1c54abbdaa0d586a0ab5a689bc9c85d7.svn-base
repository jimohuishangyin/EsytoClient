...
public class Client {
	...
	public Client build(ClientRequest clientRequest) {
		...
		if (token == null)
			token = Setting.getToken(mContext);
	...
	private String getJsonResquest(String url) throws JSONException {
		...
		// 登录，主管密码 RSA加密开通
		if (url.equals("/app/companys/login1_1")
				|| url.equals("/app/pay/checkPass1_1")) {
			try {
				byte[] en_test = RSAUtils.encrypt(
				RSAUtils.getKeyPair(RSAUtils.PublicKey(mContext), mContext)
						.getPublic(), obj.toString().getBytes("GBK"));
