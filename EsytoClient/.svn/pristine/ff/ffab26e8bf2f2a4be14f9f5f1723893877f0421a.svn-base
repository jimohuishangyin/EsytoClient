	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(context).inflate(
					R.layout.adapter_orderquery_item, null);
			holder = new ViewHolder();
			holder.mDayTextView = (TextView) convertView
					.findViewById(R.id.day_textview);
			holder.mHouseAndMinTextView = (TextView) convertView
					.findViewById(R.id.hourandmin);
			holder.mPaymentImageView = (ImageView) convertView
					.findViewById(R.id.payment_imageview);
			holder.mPaymentTextView = (TextView) convertView
					.findViewById(R.id.payment_textview);
			holder.mOrderMothedTextView = (TextView) convertView
					.findViewById(R.id.ordertextview);
			holder.mMoneyTextView = (TextView) convertView
					.findViewById(R.id.money_textview);
			convertView.setTag(holder);
		} else {
			holder = (ViewHolder) convertView.getTag();
		}
		ObjList item = (ObjList) getItem(position);
		holder.mPaymentTextView.setText(mPayTypeEnTity.getPayName(item
				.getChannelType()));
		...
		if (item != null) {
			...
			holder.mPaymentTextView.setText(mPayTypeEnTity
					.getPayName(channelType));
			holder.mPaymentImageView.setImageResource(mPayTypeEnTity
					.get96ImgId(channelType));
			if (status.equals("1")) {
				...
				holder.mOrderMothedTextView.setTextColor(context.getResources()
						.getColor(R.color.black_1));
				holder.mMoneyTextView.setText(amount);
				holder.mMoneyTextView.setTextColor(context.getResources()
						.getColor(R.color.black_1));
