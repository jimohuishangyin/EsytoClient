...
public class StoreDetailActivity extends BaseActivity implements
	...
	private void getShopMsg() {
		...
		task.setOnTaskFinished(new OnTaskFinished() {
			...
			public void onSucc(Object obj) {
				...
				headcount = String.valueOf(response.getHeadcount());
				managersList.clear();
	 ...
	 * @throws
	 */
	private void updateShopMsg(final int code) {
		...
		task.setOnTaskFinished(new OnTaskFinished() {
			...
			public void onSucc(Object obj) {
				...
				if(code==1){
					// 跳转到员工
					Intent headcountIntent = new Intent(mContext,
							PersonalManagerActivity.class);
	...
	private void showNamePopwindow(String hint, final int state) {
		...
		if (state == 0) {
			tvName.setFilters(new InputFilter[] { new InputFilter.LengthFilter(
					100) });
			tvName.setText(shopName);
		} else if (state == 1) {
			tvName.setFilters(new InputFilter[] { new InputFilter.LengthFilter(
					150) });
			tvName.setText(shopAddress);
		} else if (state == 2) {
			tvName.setFilters(new InputFilter[] { new InputFilter.LengthFilter(
					12) });
		}
