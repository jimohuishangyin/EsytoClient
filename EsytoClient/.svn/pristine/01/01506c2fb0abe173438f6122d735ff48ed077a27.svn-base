...
public class DeviceListDialog extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (pairedDevices.size() > 0) {
			for (BluetoothDevice device : pairedDevices) {
				if(MyApplication.getBuleTooth(DeviceListDialog.this)==null){
					mPairedDevicesArrayAdapter.add(device.getName() + "\n"
							+ device.getAddress());
				}else {
					if ((MyApplication.getBuleTooth(DeviceListDialog.this)).equals(device.getAddress())){
	...
	private void initView() {
		...
		lv_paired_devices = (ListView) this.findViewById(R.id.lv_paired_devices);
		lv_paired_devices.setAdapter(mPairedDevicesArrayAdapter);
		lv_paired_devices.setOnItemClickListener(mDeviceClickListener);
		lv_new_devices = (ListView) this.findViewById(R.id.lv_new_devices);
