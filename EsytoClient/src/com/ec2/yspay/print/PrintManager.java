...
public class PrintManager {
	...
	public static final int BLUETOOTH_ENABLE_QR = 0;
	...
	public void printTotal(PrintTotalEntity entity) {
		...
		sendMessage("日    期："
				+ DateUtils.dateToStr("yyyy-MM-dd", entity.gettDate()) + "\n");
		sendMessage("打印时间："
				+ DateUtils.dateToStr("yyyy-MM-dd HH:mm:ss", entity.gettDate())
				+ "\n\n");
		sendMessage("现    金：" + entity.getCashMoney() + "元（"
				+ entity.getCashCount() + "笔）\n");
		sendMessage("银 行 卡：" + entity.getCardMoney() + "元（"
				+ entity.getCardCount() + "笔）\n");
		sendMessage("翼 支 付：" + entity.getBestMoney() + "元（"
				+ entity.getBestCount() + "笔）\n");
		sendMessage("支 付 宝：" + entity.getAliMoney() + "元（"
				+ entity.getAliCount() + "笔）\n");
		sendMessage("微信支付：" + entity.getWxMoney() + "元（" + entity.getWxCount()
				+ "笔）\n\n");
		sendMessage("总金额：" + entity.getTotalAmount() + "元（"
				+ entity.getTotalItems() + "笔）\n\n\n");
	...
	public void printDetail(PrintDetailEntity entity) {
		...
		for (int i = 0; i < mList.size(); i++) {
			sendMessage(mList.get(i).getOrderNo() + " "
					+ mList.get(i).getOrderTime().substring(11, 16) + " "
					+ mList.get(i).getAmount() + "\n");
		}
		sendMessage("\n");
		sendMessage("总金额：" + entity.getTotalAmount() + "元" + "（"
				+ entity.getTotalItems() + "笔）" + "\n");
	...
	public void printOneDetail(PrintOrderEntity mPrintOrderEntity) {
		...
		sendMessage("支付方式："
				+ PayTypeEntity.getPayName(mPrintOrderEntity.getChannelType())
				+ "\n");
	...
	private String spitAccount(String account) {
		String phoneNum = "";
		phoneNum = account.substring(0, 3) + "*****"
	...
	private void showWaitingDialog() {
		final PayLoadingDialog.Builder builder = new PayLoadingDialog.Builder(
				mContext);
		builder.setContentMsg("蓝牙连接中......");
		builder.setPositiveButton("取消打印",
	...
	public void showDeviceDialog() {
		...
		final DeviceListDialog.Builder builder = new DeviceListDialog.Builder(
				mContext);
