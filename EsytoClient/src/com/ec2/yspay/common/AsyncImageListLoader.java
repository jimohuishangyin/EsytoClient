...
public class AsyncImageListLoader {
	...
	public AsyncImageListLoader(Context context) {
	...
	public void loadImage(ImageView imageView, String imageUrl,String fileName,Drawable defaultDrawable){
		imageView.setTag(fileName);
		imageView.setImageDrawable(defaultDrawable);
		if (!TextUtils.isEmpty(imageUrl)) {
			Bitmap bitmap = loadImage(imageView, imageUrl, fileName);
			if (bitmap != null) {
	 ...
	 * @param imageUrl
	 */
	public Bitmap loadImage(ImageView imageView, String imageUrl,String fileName) {
		// 先从内存中拿
		Bitmap bitmap = getBitmapFromMem(fileName);
		...
		// 再从文件中找
		bitmap = getBitmapFromDisk(fileName);
		if (bitmap != null) {
			Log.i("leslie", "image exists in file");
			// 重新缓存到内存中
			putBitmapToMem(fileName, bitmap);
			return bitmap;
		}
	...
	class ImageDownloadTask extends AsyncTask<String, Integer, Bitmap> {
		...
		protected Bitmap doInBackground(String... params) {
			try {
				...
				Bitmap bitmap = getBitmapFromDisk(fileName);
				if (bitmap != null) {
					// 将图片加入到内存缓存中
					putBitmapToMem(fileName, bitmap);
				}
		...
		protected void onPostExecute(Bitmap result) {
			...
			if (result != null) {
				// 通过 tag 来防止图片错位
				if (imageView.getTag() != null && imageView.getTag().equals(fileName)) {
