...
//        }
     ...
     * 保存Bug奔溃日志到文件
     * @author zhangyi
     * @createTime 2014-11-4 下午2:08:12
     */
    public static void saveCrashLogcat(MyCrashHandler instance,Throwable ex) {
        String status = Environment.getExternalStorageState();
        if (status.equals(Environment.MEDIA_MOUNTED)) {
            if (instance != null) {
                try {
                    // 将crash log写入文件
                    File filename = new File(Constants.PATH_FILE+"/log");
                    if (!filename.exists()) {
                        filename.mkdirs();
                    }
    ...
    /*
     * 
     * 卓仲明　２０１５.１２.１１自定义弹出框
     * 
     * */
    public static void esytoToast(Context mContext,String content){
    	Toast toast=Toast.makeText(mContext, content, Toast.LENGTH_SHORT);
    	toast.setGravity(Gravity.BOTTOM, 0, 250);
    	toast.getView().setBackgroundResource(R.drawable.tanchuang);
    	toast.show();
    }
    public static void esytoLongToast(Context mContext,String content){
