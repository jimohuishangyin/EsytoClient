...
public class ReportFormEntity
{
    ...
    public String baiduMoney="0.00",baiduCount="0";
    ...
    public float getTotalMoney(){
        ...
        total+=Float.valueOf(baiduMoney);
        return total;
    }
    private int getTotalCount(){
    	int total = 0;
    	total+=Integer.valueOf(cashCount);
    	total+=Integer.valueOf(cardCount);
    	total+=Integer.valueOf(bestCount);
    	total+=Integer.valueOf(aliCount);
    	total+=Integer.valueOf(wxCount);
    	total+=Integer.valueOf(baiduCount);
    	return total ;
    }
    public float getCountPercent(String count){
    	int temp = Integer.valueOf(count);
    	return temp*100.0f/getTotalCount();
    }
    /**
     * @author   罗洪祥
     * @version  V001Z0001
        ...
        for(PaytypeCountItem item:mList){
            ...
            switch (payId)
            {
                ...
                case PayTypeEntity.PAY_BAIDU:
                	baiduMoney = item.getTotalAmount();
                    baiduCount = item.getTotalItems();
                    break;
                default:
                    break;
            }
